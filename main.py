...
for i in list_of_percent_train_data:
    print(i)
    train_data, test_data = split_random_to_train_and_test_data(row_attrs, class_vals, i)
    decision_tree = DecisionTree(train_data, MAX_DEPTH, method='entropy', use_FEM=True)
    acc = decision_tree.calculate_acc(test_data)
    list_of_acc.append(acc)
    print(acc)
    decision_tree = DecisionTree(train_data, MAX_DEPTH, method='gini', use_FEM=True)
    acc = decision_tree.calculate_acc(test_data)
    list_of_acc.append(acc)
    labels_for_percent_of_train_data.append('%.2f%%' % (i))
    print(acc)
...
for i in list_of_percent_train_data:
    ...
    acc = decision_tree.calculate_acc(test_data)
    cm = confusion_matrix(test_data[1], [decision_tree.predict_decision_tree(row, use_voting=False) for i, row in enumerate(test_data[0]["attrs_vals"])])
    f1 = f1_score(test_data[1], [decision_tree.predict_decision_tree(row, use_voting=False) for i, row in enumerate(test_data[0]["attrs_vals"])], average='weighted')
    list_of_acc.append(acc)
    list_of_f1.append(f1)
    print(acc)
    print(f1)

    decision_tree = DecisionTree(train_data_mode, MAX_DEPTH, default_prediction, method='gini', use_FEM=False)
    acc = decision_tree.calculate_acc(test_data_mode)
    cm = confusion_matrix(test_data_mode[1], [decision_tree.predict_decision_tree(row, use_voting=True) for i, row in enumerate(test_data_mode[0]["attrs_vals"])])
    f1 = f1_score(test_data_mode[1], [decision_tree.predict_decision_tree(row, use_voting=True) for i, row in enumerate(test_data_mode[0]["attrs_vals"])], average='weighted')
    list_of_acc.append(acc)
    list_of_f1.append(f1)
    print(acc)
    print(f1)
